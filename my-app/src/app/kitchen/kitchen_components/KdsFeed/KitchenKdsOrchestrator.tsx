...
// RESPONSIBILITY: Orchestrates KDS (Kitchen Display System) UI logic, tab states, and modals.
// Renders KitchenKpiSummaryBar, KitchenKotGrid, KitchenCompletedOrdersView, KitchenStockToggle, etc.
// DATA FLOW: KDS UI Logic

import { useState, useEffect, useMemo, useRef } from "react";
import { Search, Filter, History, Layers } from "lucide-react";
...
import {
  KITCHEN_PAGE_TITLE,
  KITCHEN_PAGE_SUBTITLE,
  STATION_TABS,
  STATION_TAB_LABELS,
  STOCK_TAB_KEY,
  COMPLETED_TAB_KEY,
  KITCHEN_SKELETON_COUNT,
} from "@/app/kitchen/kitchen_constants/kitchen_shared_constants";

type PageTab = KitchenStationTab | typeof STOCK_TAB_KEY | typeof COMPLETED_TAB_KEY;

export function KitchenKdsOrchestrator() {
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Array.from({ length: KITCHEN_SKELETON_COUNT }).map((_, i) => (
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex flex-col gap-1">
        <h1 className="text-[22px] font-bold text-text-primary">{KITCHEN_PAGE_TITLE}</h1>
        <p className="text-sm text-text-secondary">{KITCHEN_PAGE_SUBTITLE}</p>
      </div>
