...
import { AdminRecipeEditor } from "@/app/admin/menu/admin_menu_components/AdminRecipeEditor";
import { AdminMenuVariantsList } from "@/app/admin/menu/admin_menu_components/AdminMenuVariantsList";
...
export function AdminMenuFormModal({
  ...
  return (
    <div
      className="fixed inset-0 z-40 flex items-start justify-center overflow-y-auto bg-overlay p-4 pt-10"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-text-secondary hover:bg-page motion-safe:transition-colors"
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-4 px-5 py-5">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="rounded-lg border border-border bg-input px-3 py-2 text-[13px] text-text-primary placeholder:text-text-disabled focus:border-border-focus focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            />
            {errors.name && (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                min={0}
                value={watch("price") || ""}
                onChange={(e) => setValue("price", e.target.value ? Number(e.target.value) : 0, { shouldValidate: true, shouldDirty: true })} onKeyDown={(e) => { if (e.key === "-" || e.key === "e" || e.key === "E") e.preventDefault(); }}
                placeholder="280"
                className="rounded-lg border border-border bg-input px-3 py-2 text-[13px] text-text-primary placeholder:text-text-disabled focus:border-border-focus focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              />
              {errors.price && (
          ...
          <div className="flex flex-col gap-1">
            ...
            <select
              {...register("station")}
              className="rounded-lg border border-border bg-input px-3 py-2 text-[13px] text-text-primary focus:border-border-focus focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            >
              {STATIONS.map((s) => (
          ...
          {/* Variants */}
          <AdminMenuVariantsList fields={fields} append={append} remove={remove} register={register} />
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 text-[14px] font-semibold text-white hover:bg-primary-hover disabled:opacity-60 motion-safe:transition-colors"
          >
            {isSubmitting && <Loader2 size={15} className="motion-safe:animate-spin" />}
