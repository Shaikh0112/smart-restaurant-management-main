...
## Directory Structure
- **[Feature Folders] (e.g., `menu/`, `inventory/`)**: Each domain has its own folder containing its `page.tsx`, `error.tsx`, `loading.tsx`, and a sub-folder for its specific components.
- **[Feature]_components/**: Components are now co-located with their feature (e.g., `menu/admin_menu_components/AdminMenuTable.tsx`) following Domain-Driven Design (Rule 1).
- **admin_components/**: Now strictly reserved for globally shared UI elements within the admin module (e.g., `AdminActionDialog.tsx`).
- **admin_hooks/**: Custom hooks for business logic and state management.
- **admin_types/**: Centralized TypeScript interfaces to ensure strict Prop Blueprints.
