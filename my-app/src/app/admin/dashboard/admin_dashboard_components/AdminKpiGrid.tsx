...
function AdminKpiCard({ id: _id, label, value, icon: Icon, trend, trendUp }: AdminKpiCardData) {
  ...
  const isActive = searchParams.get("kpi") === _id;
  ...
  return (
    <button 
      onClick={handleClick}
      className={[
        "flex w-full flex-col gap-3 rounded-lg border bg-card p-4 text-left motion-safe:transition-all motion-safe:duration-200 hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-primary",
