...
interface CustomerReservationBookingFormProps {
  bookingDate: string;
...
export function CustomerReservationBookingForm({
  bookingDate,
  setBookingDate,
  bookingTime,
  setBookingTime,
  ...
  perPersonRate,
  onPayClick,
}: CustomerReservationBookingFormProps) {
  return (
    <div className="rounded-3xl border border-border/80 bg-card p-6 shadow-xl flex flex-col gap-6">
      <div className="flex items-center justify-between border-b border-border/50 pb-3">
        ...
        <span className="rounded-full bg-success/10 border border-success/30 px-3 py-1 text-xs font-bold text-success">
          ₹{perPersonRate} / Guest Deposit
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <div className="relative">
            <Clock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
            <input
              type="time"
              value={bookingTime}
      ...
      <div className="rounded-2xl border border-success/40 bg-success/10 p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <p className="text-xs text-text-muted">Calculated Advance Deposit</p>
          <p className="font-black text-2xl text-success">
            ₹{totalAdvanceDeposit}{" "}
            <span className="text-xs font-bold text-text-secondary">
              ({guestCount} Persons × ₹{perPersonRate})
            </span>
          </p>
        ...
        <button
          type="button"
          onClick={onPayClick}
          className="flex items-center gap-2 rounded-2xl bg-success px-6 py-3.5 font-black text-xs text-white shadow-xl hover:bg-success-hover active:scale-95 transition-all w-full sm:w-auto justify-center"
        >
          <CreditCard size={18} />
