...
// RESPONSIBILITY: Component rendering JobsQueueTable

import React from "react";
import type { BackgroundJob } from "@/app/super-admin/super-admin_types/jobs_types";
...
export default function JobsQueueTable({ jobs, onRetry, isRetrying }: Props) {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'processing': return 'bg-primary/10 text-text-primary';
  ...
  return (
    <div className="bg-card border border-border rounded-lg overflow-hidden">
      <div className="p-4 border-b border-border flex justify-between items-center bg-page/50">
        <h2 className="text-base font-bold text-text-primary">Active Worker Queues</h2>
        <button className="bg-secondary/10 text-text-primary px-4 py-2 rounded-md text-body font-medium hover:bg-secondary/20 motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page">
          Pause Queues
        </button>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-body">
            {jobs.map((job) => (
              <tr key={job.id} className="border-b border-border hover:bg-page/50 motion-safe:transition-colors">
                <td className="p-4">
                  <p className="font-medium text-text-primary">{job.name}</p>
                  <p className="text-badge text-text-secondary font-mono">{job.id}</p>
                </td>
                <td className="p-4">
                  <span className="px-2 py-1 bg-page rounded-md border border-border text-table-header text-text-primary">
                    {job.queue}
                  </span>
                </td>
                <td className="p-4">
                  <div className="flex flex-col gap-1 items-start">
                    <span className={`px-2.5 py-1 rounded-full text-table-header font-medium capitalize ${getStatusColor(job.status)}`}>
                      {job.status}
                    </span>
                    {job.error && (
                      <span className="text-badge text-danger max-w-[150px] truncate flex items-center gap-1" title={job.error}>
                        <AlertCircle size={10} strokeWidth={2} /> {job.error}
                      </span>
                    )}
                  </div>
                </td>
