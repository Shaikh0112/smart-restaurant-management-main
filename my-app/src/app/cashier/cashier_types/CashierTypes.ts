...
export interface CashierTaxRowProps {
  label: string;
  value: number;
}

export interface CashierReceiptItemRowProps {
  name: string;
  qty: number;
  totalPrice: number;
}
...
import { FetchState } from "../cashier_hooks/useTransaction";

export interface UseCashierCheckoutReturn {
  status:            FetchState;
  processCheckout:   (payload: CashierCheckoutPayload, config?: { restaurantName?: string; waiterRoute?: string; cashierId?: string }) => Promise<boolean>;
  buildWhatsAppLink: (phone: string, receiptText: string) => string;
  buildReceiptText:  (cartItems: CashierCartItem[], taxBreakdown: CashierTaxBreakdown, tableNumber: string, restaurantName?: string) => string;
}
