...
const SINGLE_METHODS: { value: CashierSingleMethod; label: string; icon: React.ReactNode }[] = [
  { value: "CASH", label: "Cash", icon: <Banknote size={18} strokeWidth={2} />    },
  { value: "UPI",  label: "UPI",  icon: <Smartphone size={18} strokeWidth={2} />  },
  { value: "CARD", label: "Card", icon: <CreditCard size={18} strokeWidth={2} />  },
];
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-border bg-card p-4">
          ...
          {SINGLE_METHODS.map((sm) => (
            <button
              ...
              className={[
                ...
                "text-sm font-medium motion-safe:transition-colors duration-150",
      ...
      {mode === "SPLIT_BILL" && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3">
            <label htmlFor="guest-count" className="text-sm text-text-secondary">
              Number of guests:
            </label>
            <input id="guest-count"
              type="number"
              min={MIN_GUESTS}
              max={MAX_GUESTS}
              value={guestCount}
      ...
      {mode === "SPLIT_PAYMENT" && (
        <div className="flex flex-col gap-3">
          {[
            { label: "Cash (₹)",  value: splitCash, setter: setSplitCash,  icon: <Banknote size={18} strokeWidth={2} />   },
            { label: "UPI (₹)",   value: splitUpi,  setter: setSplitUpi,   icon: <Smartphone size={18} strokeWidth={2} /> },
            { label: "Card (₹)",  value: splitCard, setter: setSplitCard,  icon: <CreditCard size={18} strokeWidth={2} /> },
          ].map((field) => (
            <div key={field.label} className="flex items-center gap-3">
              <div className="flex w-24 items-center gap-1.5 text-xs text-text-secondary">
                {field.icon}
                {field.label}
              </div>
              <input type="number"
