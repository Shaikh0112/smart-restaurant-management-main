...
// DATA FLOW: cartItems + taxBreakdown -> CashierThermalReceiptPreviewModal -> Thermal UI Preview / Window Printimport React from "react";
import { Printer, X } from "lucide-react";
import { formatCurrency, formatDateTime } from "@/lib/formatters";
import type { CashierCartItem, CashierTaxBreakdown } from "@/app/cashier/cashier_types/CashierTypes";
import { CashierPrintableWrapper } from "./CashierPrintableWrapper";
...
export function CashierThermalReceiptPreviewModal({
  ...
  const useRestaurantSettings = () => {
    return {
      restaurantName: "Dynamic Restaurant Name",
      restaurantAddress: "Dynamic Address",
      gstin: "Dynamic GSTIN",
    };
  };
  const { restaurantName, restaurantAddress, gstin } = useRestaurantSettings();
  ...
  return (
    <div className="fixed inset-0 z-[9990] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in">
      <CashierPrintableWrapper onPrint={handlePrint} onClose={onClose} copyId={`TBL-${tableNumber}-${Date.now().toString().slice(-4)}`}>
        {/* Modal Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-zinc-950 flex justify-center">
          <div className="w-full max-w-[280px] bg-white text-black font-mono p-4 rounded-md shadow-2xl text-xs space-y-2 border-dashed border-b-2 border-zinc-400">
            ...
            <div className="text-center space-y-1 pb-2 border-b border-dashed border-zinc-300">
              ...
              <p className="text-xs text-zinc-600 leading-tight">{restaurantAddress}</p>
              <p className="text-xs font-semibold text-zinc-700">GSTIN: {gstin}</p>
            </div>
            ...
            {/* Footer */}
            <div className="text-center pt-3 text-xs text-zinc-600 space-y-1 border-t border-dashed border-zinc-300">
              <p>Thank you for dining with us! </p>
              <p className="font-bold text-black">Visit Again soon!</p>
            </div>
          </div>
        </div>
      </CashierPrintableWrapper>
    </div>
  );
}
