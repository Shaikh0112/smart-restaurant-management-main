...
export function CashierApprovalCenterModal({
  ...
  const [confirmingVoid, setConfirmingVoid] = React.useState<string | null>(null);
  const [confirmVoidText, setConfirmVoidText] = React.useState("");
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {pendingVoidRequests.length === 0 ? (
            ...
            pendingVoidRequests.map((req) => (
              <div
                key={`${req.orderId}-${req.kotId}-${req.itemId}`}
                className="flex flex-col gap-3 rounded-xl border border-red-500/30 bg-red-500/5 p-4 shadow-xs"
              >
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  {confirmingVoid === `${req.orderId}-${req.kotId}-${req.itemId}` ? (
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        ...
                        className="w-24 rounded-lg border border-red-500/50 bg-input py-1 px-2 text-xs font-bold uppercase focus:border-red-500 focus:outline-none"
                      />
                      <button
                        onClick={() => {
                          if (confirmVoidText === "VOID") {
                            handleDecision(req.orderId, req.kotId, req.itemId, req.tableNumber, true);
                            setConfirmingVoid(null);
                            setConfirmVoidText("");
                          }
                        }}
                        disabled={confirmVoidText !== "VOID"}
                        className="rounded-xl bg-red-500 px-3.5 py-1.5 text-xs font-bold text-white hover:bg-red-600 shadow-sm disabled:opacity-50"
                      >
                        Confirm Void
                      </button>
                      <button
                    ...
                    <button
                      onClick={() => setConfirmingVoid(`${req.orderId}-${req.kotId}-${req.itemId}`)}
