 ...
 * Print button triggers // Extracted print mechanism handled by ClientWrapper 
      window.print() — @media print hides chrome.
 */
  ...
  return (
    <div className="flex flex-col gap-6" id="shift-z-report">
      ...
      <div className="flex justify-end print:hidden">
        <button
          onClick={() => // Extracted print mechanism handled by ClientWrapper 
      window.print()}
          className="flex items-center gap-2 rounded-xl border border-border px-4 py-2 text-sm font-semibold text-text-secondary hover:bg-card motion-safe:transition-colors"
        >
              ...
              <thead>
                <tr className="border-b border-border">
                  {["Waiter ID", "Orders Served", "Total Sales"].map((h) => (
                    <th key={h} className="pb-2 text-xs font-semibold uppercase tracking-wide text-text-secondary">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {Object.entries(shift.waiterStats).map(([waiterId, stats]) => (
                  <tr key={waiterId} className="border-b border-border last:border-0">
                    <td className="py-2.5 text-sm font-medium text-text-primary">{waiterId}</td>
                    <td className="py-2.5 text-sm text-text-secondary">{stats.ordersServed}</td>
                    <td className="py-2.5 text-sm font-medium text-text-primary">{formatCurrency(stats.totalSales)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
