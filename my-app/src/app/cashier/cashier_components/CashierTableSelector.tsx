...
import { TableEmptyState } from "@/app/cashier/cashier_components/CashierEmptyStates";
...
export function CashierTableSelector({
  ...
  const orderType = (searchParams.get("type") as OrderTypeTab) || "Dine-In";
  ...
  const setOrderType = (val: OrderTypeTab) => updateParam("type", val);
  const setViewMode = (val: "grid" | "list") => updateParam("view", val);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-1.5 rounded-2xl border border-border bg-card p-1 shadow-xs">
        <button
          ...
          className={`flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-bold motion-safe:transition-all ${
            orderType === "Dine-In"
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center rounded-xl border border-border bg-card p-1">
          <button
            ...
            className={`rounded-lg p-1 motion-safe:transition-colors ${
          ...
          {filteredTables.map(({ table, order }) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col justify-between rounded-2xl border p-3 text-left motion-safe:transition-all active:scale-95 shadow-xs ${
                  isSelected
              ...
              >
                <div className="flex items-start justify-between gap-1">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[0.5rem] font-black uppercase flex items-center gap-1 ${
                      isBillRequested
                ...
                <div className="mt-2 text-xs text-text-secondary flex justify-between items-center border-t border-border/40 pt-1.5">
          ...
          {filteredTables.map(({ table, order }) => {
            ...
            return (
              <li key={table.id}>
                <button
                  onClick={() => onSelect(table.id)}
                  className={`w-full rounded-2xl border px-4 py-3 text-left motion-safe:transition-all ${
                    isSelected
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${sectionBadge}`}>
                      {table.section}
                    </span>
                  </div>
