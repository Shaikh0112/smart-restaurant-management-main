...
export function CashierShiftReconciliationModal({
  ...
  async function handleFinalizeClosing() {
    if (confirmText !== "RECONCILE") return;
        ...
        {!isClosed ? (
          <div className="mt-5 flex flex-col gap-5 text-xs">
            ...
            <div className="grid grid-cols-3 gap-3 rounded-xl border border-border/50 bg-page p-3 text-center">
              <div>
                <span className="text-xs text-text-secondary font-medium">Opening Float</span>
                <p className="text-sm font-extrabold text-text-primary">{formatCurrency(metrics.openingFloat)}</p>
              </div>
              <div>
                <span className="text-xs text-text-secondary font-medium">Shift Cash Sales</span>
                <p className="text-sm font-extrabold text-success">{formatCurrency(metrics.cashCollected)}</p>
              </div>
              <div>
                <span className="text-xs text-text-secondary font-medium">Expected Cash Till</span>
            ...
            <div className="flex flex-col gap-2">
              <h3 className="font-bold text-text-primary uppercase text-xs flex items-center gap-1.5">
                <Calculator size={18} strokeWidth={2} className="text-primary" /> Physical Cash Denomination Counter
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 rounded-xl border border-border/50 bg-surface p-3">
                {[
                  ...
                  <div key={den.key} className="flex flex-col gap-1 rounded-lg border border-border/40 bg-page p-2">
                    <span className="font-bold text-text-primary text-xs">{den.label}</span>
            ...
            <div className="flex items-center justify-between rounded-xl border border-border bg-card p-3.5 shadow-xs">
              <div className="flex flex-col gap-0.5">
                <span className="text-xs uppercase font-bold text-text-secondary">Physical Cash Count</span>
                <span className="text-lg font-black text-text-primary">{formatCurrency(physicalCashTotal)}</span>
              </div>

              <div className="flex flex-col items-end gap-0.5">
                <span className="text-xs uppercase font-bold text-text-secondary">Discrepancy (Variance)</span>
                <span
                  className={[
                    "text-lg font-black",
                    discrepancy === 0
          ...
          <div className="mt-5 flex flex-col items-center justify-center gap-4 py-8 text-center">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              >
                <Printer size={18} strokeWidth={2} /> Print Z-Report Slip
              </button>
              <button
                onClick={onClose}
          ...
          <div className="mt-5 flex flex-col gap-3 border-t border-border/60 pt-4">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-text-secondary uppercase">
                Type "RECONCILE" to finalize Z-Report
              </label>
              <input
                type="text"
                value={confirmText}
                ...
                className="w-full rounded-lg border border-border bg-input py-2 px-3 text-xs font-bold text-text-primary focus:outline-none focus:border-primary uppercase"
              />
            </div>
            <div className="flex justify-end gap-2">
              <button
                ...
                className="rounded-lg border border-border bg-page px-4 py-2 font-bold text-text-primary hover:bg-surface text-xs disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                onClick={handleFinalizeClosing}
                disabled={confirmText !== "RECONCILE" || isSubmitting}
                className="flex items-center gap-1.5 rounded-lg bg-danger px-4 py-2 font-bold text-white hover:bg-danger/90 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Lock size={18} strokeWidth={2} /> {isSubmitting ? "Processing..." : "Finalize Z-Report Closing"}
              </button>
            </div>
          </div>
        )}
