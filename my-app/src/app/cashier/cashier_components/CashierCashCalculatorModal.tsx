...
export function CashierCashCalculatorModal({
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="flex h-auto w-full max-w-md flex-col overflow-hidden rounded-3xl border border-border bg-surface text-text-primary shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[500, 1000, 2000].map((amt) => (
                <button
                  ...
                  className={`rounded-xl border p-2.5 text-xs font-extrabold motion-safe:transition-all ${
                    tendered === amt
          ...
          <div
            className={`rounded-2xl border p-4 space-y-1 text-center motion-safe:transition-all ${
              isShortage
        ...
        <div className="p-5 pt-0">
          ...
          <input
            ...
            placeholder="PAY"
            className="w-full rounded-2xl border border-border bg-input py-3 px-4 text-sm font-bold text-text-primary focus:border-primary focus:outline-none uppercase"
          />
        </div>
        ...
        <div className="flex justify-end gap-3 border-t border-border p-4 bg-page/50">
          <button
            ...
            className="rounded-xl border border-border px-4 py-2 text-xs font-bold text-text-secondary hover:bg-page disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={handleConfirmAction}
            disabled={isShortage || confirmText !== "PAY" || isSubmitting}
            className="flex items-center gap-2 rounded-xl bg-emerald-500 px-5 py-2.5 text-xs font-bold text-white shadow-md hover:bg-emerald-600 active:scale-95 motion-safe:transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <span>{isSubmitting ? "Processing..." : "Confirm & Complete Settle"}</span>
