...
// Prevents unauthorized cashier overrides without Manager PIN.
...
// Mock API function for PIN validation
const mockVerifyManagerPin = async (pin: string): Promise<boolean> => {
  const res = await cashierApiClient.post('/auth/verify-manager-pin', { pin });
  return res.success;
};
...
export function CashierManagerPinModal({
  ...
  const handleVerify = async () => {
    ...
    setIsVerifying(true);
    try {
      const isValid = await mockVerifyManagerPin(pin);
      if (isValid) {
        ...
        <div className="p-5 flex flex-col items-center gap-4 text-center">
          ...
          <div className={`flex items-center justify-center gap-3 my-2 ${isError ? "animate-shake" : ""}`}>
            {[0, 1, 2, 3].map((idx) => (
              <div
                key={idx}
                className={`h-4 w-4 rounded-full border-2 motion-safe:transition-all ${
                  pin.length > idx
          ...
          <div className="grid grid-cols-3 gap-3.5 w-full max-w-[240px]">
            {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map((num) => (
              <button
                ...
                className="flex h-12 w-full items-center justify-center rounded-2xl border border-border bg-page text-lg font-bold text-text-primary hover:bg-surface-hover hover:border-primary/50 active:scale-95 motion-safe:transition-all shadow-xs"
              >
                {num}
              </button>
            ))}
            <button
              ...
              className="flex h-12 w-full items-center justify-center rounded-2xl border border-border bg-page text-text-muted hover:text-red-500 hover:border-red-500/40 active:scale-95 motion-safe:transition-all"
            >
              <Delete className="h-5 w-5" />
            </button>
            <button
              onClick={() => handleKeyPress("0")}
              className="flex h-12 w-full items-center justify-center rounded-2xl border border-border bg-page text-lg font-bold text-text-primary hover:bg-surface-hover active:scale-95 motion-safe:transition-all"
            >
              0
            </button>
            <button
              ...
              className="flex h-12 w-full items-center justify-center rounded-2xl bg-primary text-white font-bold hover:bg-primary-hover active:scale-95 motion-safe:transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-md shadow-primary/20"
            >
