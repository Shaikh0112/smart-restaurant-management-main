  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm">
      <div className="flex items-center gap-2 border-b border-border/50 pb-2">
        <SlidersHorizontal size={18} strokeWidth={2} className="text-primary" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-text-secondary flex items-center gap-1">
            <Package size={18} strokeWidth={2} className="text-info" /> Parcel / Packaging Fee (â‚¹)
          </label>
          <div className="flex items-center gap-1.5">
            {PACKAGING_PRESETS.map((fee) => (
              <button
                ...
                className={[
                  "rounded-lg px-2.5 py-1 text-xs font-bold border motion-safe:transition-colors",
                  packagingCharge === fee
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-text-secondary flex items-center gap-1">
            <HeartHandshake size={18} strokeWidth={2} className="text-success" /> Customer Staff Tip (â‚¹)
          </label>
          <div className="flex items-center gap-1.5">
            {TIP_PRESETS.map((tip) => (
              <button
