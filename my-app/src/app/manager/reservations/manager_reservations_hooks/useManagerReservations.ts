...
// RESPONSIBILITY: All data and mutation logic for the ManagerReservations page.
// Reads/writes RESERVATIONS + TABLES from localStorage via useLocalStorage.
// Derives upcoming and past lists via useMemo.
// No JSX — pure logic hook consumed by manager_reservations/page.tsx.
// DATA FLOW: localStorage → useLocalStorage → useManagerReservations
...
import type {
  ManagerReservationsFormValues,
  UseOwnerReservationsReturn,
} from "@/app/manager/reservations/manager_reservations_types/ManagerReservationsTypes";
 ...
 * upcoming/past lists are memoized and recalculate only when manager_reservations change.
 *
 * @returns upcoming, past, availableTables, isSubmitting, cancellingId, handlers
 */
export function useManagerReservations(): UseOwnerReservationsReturn {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [cancellingId, setCancellingId] = useState<string | null>(null);

  // Rule 61: No direct localStorage — hooks only
  const [manager_reservations, setOwnerReservations] = useLocalStorage<AppReservation[]>(
  ...
  // Deps: manager_reservations — recompute when list changes
  const now = Date.now();
   ...
   *
   * @param values - Validated form values from ManagerReservationsFormModal
   */
  const addReservation = useCallback(
    (values: ManagerReservationsFormValues) => {
  ...
  const cancelReservation = useCallback(
    (id: string) => {
      ...
      const target = manager_reservations.find((r) => r.id === id);
    ...
    [manager_reservations, setOwnerReservations, setTables]
  );
