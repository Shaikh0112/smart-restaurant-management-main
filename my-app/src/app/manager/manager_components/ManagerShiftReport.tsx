...
// DATA FLOW: useManagerShift → admin/shift/page.tsx → ManagerShiftReport → UI

import { Printer, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react";
import { useRouter, usePathname, useSearchParams } from "next/navigation";
import { formatCurrency, formatDateTime } from "@/lib/formatters";
import type { ManagerShiftReportProps } from "@/app/manager/manager_types/ManagerTypes";
 ...
 */
export function ManagerShiftReport({ shift, salesHistory }: ManagerShiftReportProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  // Sorting
  const sortParam = searchParams.get("sort") || "waiterId_asc";
  ...
  const sortedWaiterStats = Object.entries(shift.waiterStats).sort((a, b) => {
    const [idA, statsA] = a;
    const [idB, statsB] = b;
    
    let comparison = 0;
    if (sortKey === "waiterId") comparison = idA.localeCompare(idB);
    if (sortKey === "ordersServed") comparison = statsA.ordersServed - statsB.ordersServed;
    if (sortKey === "totalSales") comparison = statsA.totalSales - statsB.totalSales;
        ...
        <div className="rounded-xl border border-border bg-card px-5 py-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
                ...
                <tr className="border-b border-border">
                  {[
                    ...
                    <th 
                      ...
                      className="pb-2 text-[11px] font-semibold uppercase tracking-wide text-text-secondary cursor-pointer hover:text-text-primary transition-colors select-none"
                    >
              ...
              <tbody>
                {sortedWaiterStats.map(([waiterId, stats]) => (
                  <tr 
                    key={waiterId} 
                    className="border-b border-border last:border-0 hover:bg-card/50 transition-colors cursor-default"
                    onClick={() => {}}
                  >
