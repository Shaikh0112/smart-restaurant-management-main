...
interface ManagerStaffSalaryModalProps {
  isOpen: boolean;
  staff: AppUser | null;
...
export function ManagerStaffSalaryModal({
  isOpen,
  staff,
  initialLeaveDays = 0,
  onClose,
  onConfirm,
}: ManagerStaffSalaryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs animate-fadeIn">
      <div className="w-full max-w-md rounded-2xl bg-background/60 backdrop-blur-md border border-border/50 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-6 py-5 overflow-y-auto">
          ...
          <div className="mt-1 flex flex-col gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-[11px] font-semibold text-text-secondary">Type "PAY" to confirm</label>
              <input
                type="text"
                value={confirmText}
                onChange={(e) => setConfirmText(e.target.value)}
                placeholder="PAY"
                className="rounded-lg border border-border bg-input px-3 py-2 text-xs text-text-primary focus:border-primary focus:outline-none"
              />
            </div>
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={confirmText !== "PAY"}
                className="flex-1 rounded-xl bg-success py-2.5 text-[13px] font-bold text-white shadow-md hover:bg-success/90 disabled:opacity-60 transition-all active:scale-95"
              >
