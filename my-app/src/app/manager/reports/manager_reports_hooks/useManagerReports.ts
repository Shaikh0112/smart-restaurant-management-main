...
// RESPONSIBILITY: All data-fetching and calculation logic for the ManagerReports page.
...
// No JSX â€” pure logic hook consumed by manager_reports/page.tsx.
...
import type {
  ManagerReportsPeriod,
  ManagerReportsSummary,
  ManagerReportsTopItem,
  ManagerReportsDailyStat,
  UseOwnerReportsReturn,
} from "@/app/manager/reports/manager_reports_types/ManagerReportsTypes";
...
const PERIOD_LABELS: Record<ManagerReportsPeriod, string> = {
  TODAY:  "Today",
  WEEK:   "This Week",
  MONTH:  "This Month",
  ALL:    "All Time",
...
/**
 * Returns the start timestamp (midnight) for a given ManagerReportsPeriod.
 ...
 */
function getPeriodStart(period: ManagerReportsPeriod): number {
  const now = new Date();
 ...
 */
function filterByPeriod<T extends { timestamp?: number; createdAt?: number }>(records: T[], period: ManagerReportsPeriod, customDate: string): T[] {
 ...
 * @returns ManagerReportsSummary object
 */
function calcSummary(records: AppSalesRecord[], advanceRecords: AppAdvanceReservation[], period: ManagerReportsPeriod, customDate: string): ManagerReportsSummary {
 ...
 * @returns Array of ManagerReportsTopItem, max TOP_ITEMS_LIMIT entries
 */
function calcTopItems(
  ...
  const aggregated = new Map<string, ManagerReportsTopItem>();
 ...
 * @returns Array of ManagerReportsDailyStat entries
 */
function calcDailyRevenue(records: AppSalesRecord[], days: number): ManagerReportsDailyStat[] {
...
/**
 * Derives all ManagerReports page data from localStorage.
 ...
 */
export function useManagerReports(): UseOwnerReportsReturn {
  const [period, setPeriod] = useState<ManagerReportsPeriod>("MONTH");
