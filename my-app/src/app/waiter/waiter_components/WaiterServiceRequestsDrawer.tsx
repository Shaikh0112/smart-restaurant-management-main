...
import { updateServiceRequestStatus } from "@/app/waiter/waiter_utils/waiter_serviceRequestService";
...
export function WaiterServiceRequestsDrawer({
  ...
  const getSlaBadge = (createdAt: number) => {
    ...
    if (elapsedMins >= 5) {
      return (
        <span className="flex items-center gap-1 rounded-full bg-danger/15 px-2 py-0.5 text-xs font-bold text-danger animate-pulse">
  ...
  const getRequestIcon = (type: string) => {
    switch (type) {
      case "WATER":
        return <Droplets className="h-5 w-5 text-info shrink-0" />;
      case "BILL":
        return <DollarSign className="h-5 w-5 text-success shrink-0" />;
      case "CLEANING":
        return <RefreshCw className="h-5 w-5 text-purple-500 shrink-0" />;
      default:
        return <Utensils className="h-5 w-5 text-warning shrink-0" />;
    }
  };
    ...
    <div className="fixed inset-0 z-[9990] flex justify-end bg-black/50 backdrop-blur-sm animate-in fade-in">
      <div className="flex h-full w-full max-w-md flex-col bg-surface text-text-primary shadow-2xl border-l border-border animate-in slide-in-from-right duration-300">
        ...
        <div className="flex items-center justify-between border-b border-border p-4">
          ...
          <button
            onClick={onClose}
            className="rounded-lg p-1.5 text-text-muted hover:bg-page hover:text-text-primary transition-colors active:scale-95"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
                ...
                <div className="flex items-center gap-2 pt-1">
                  {req.status === "PENDING" && (
                    <button
                      ...
                      className="flex-1 rounded-xl bg-primary/10 border border-primary/30 py-2 text-xs font-bold text-primary hover:bg-primary/20 transition-colors active:scale-95"
                    >
                      Acknowledge
                    </button>
                  )}
                  <button
                    ...
                    className="flex-1 rounded-xl bg-success py-2 text-xs font-bold text-white hover:bg-success shadow-md shadow-success/20 transition-colors active:scale-95"
                  >
                    Mark Complete
                  </button>
                </div>
